...
TEST(JMP, Jump_Changes_Current_Command_And_Sets_RJ_To_Next_After_Next_Command)
{
	...
	EXPECT_CALL(listener, on_current_address_changed(_));
	EXPECT_CALL(listener, on_current_address_changed(1000)).Times(1);
...
TEST(JGE, Jumps_When_Comparison_Is_Greater_Or_Equal)
{
	...
	EXPECT_CALL(listener, on_current_address_changed(1000)).Times(1);
	EXPECT_CALL(listener, on_jump(3001)).Times(1);
	EXPECT_CALL(listener, on_current_address_changed(2000)).Times(1);
	EXPECT_CALL(listener, on_jump(1000)).Times(1);
	...
	ASSERT_EQ(2000, mix.current_address());
	ASSERT_EQ(1000, mix.rj().value());
}
